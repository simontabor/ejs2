...
EJS.prototype.parse = function(str, opts, cb) {
  ...
  for (var i = 0; i < str.length; i++) {
    ...
    if (js.trim().indexOf('include') === 0) {
      ...
      var path = self.resolveInclude(name, opts.filename);
      if (includes.indexOf(path) === -1) includes.push(path);

      // TODO: improve somehow?
      buf += "' + (function(){" + '/*-----EJSINCLUDE ' + path + '-----*/' + "})() + '";
      js = '';
    }
  ...
  for (var j = 0; j < includes.length; j++) {
    self.fetchFile(includes[j], function(err, file) {

      // should we clone the core opts object?
      var opts = { _with: false };
      opts.filename = includes[j];

      self.parse(file, opts, function(err, inc) {
        // using split + join instead of regex
        buf = buf.split('/*-----EJSINCLUDE ' + includes[j] + '-----*/').join(inc);
        isDone();
      });
    });
  }
